...
class CIFAR_CNN(GenericNeuralNet):
    ...
    def __init__(self, input_side, input_channels, conv_patch_size, hidden1_units, hidden2_units, hidden3_units, weight_decay, **kwargs):
        ...
        super(CIFAR_CNN, self).__init__(**kwargs)


    def conv2d_relu(self, input_x, conv_patch_size, input_channels, output_channels, stride):
    ...
    def inference(self, input_x):        
        ...
        with tf.variable_scope('h1_a'):
            h1_a = self.conv2d_relu(input_reshaped, self.conv_patch_size, self.input_channels, self.hidden1_units, stride=1)
            
        with tf.variable_scope('h1_c'):
            h1_c = self.conv2d_relu(h1_a, self.conv_patch_size, self.hidden1_units, self.hidden1_units, stride=2)
            
        # Hidden 2
        with tf.variable_scope('h2_a'):
            h2_a = self.conv2d_relu(h1_c, self.conv_patch_size, self.hidden1_units, self.hidden2_units, stride=1)
            
        with tf.variable_scope('h2_c'):
            h2_c = self.conv2d_relu(h2_a, self.conv_patch_size, self.hidden2_units, self.hidden2_units, stride=2)
            
        # Shared layers / hidden 3
        with tf.variable_scope('h3_a'):
            h3_a = self.conv2d_relu(h2_c, self.conv_patch_size, self.hidden2_units, self.hidden3_units, stride=1)        
        
        last_layer_units = 256
        with tf.variable_scope('h3_c'):
            h3_c = self.conv2d_relu(h3_a, 1, self.hidden3_units, last_layer_units, stride=1)

        with tf.variable_scope('dropout'):
            h3_c = tf.nn.dropout(h3_c, 0.5)
